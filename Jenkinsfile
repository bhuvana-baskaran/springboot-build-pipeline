  ...
  stages {
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan bhuvanabaskaran/democicd:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 bhuvanabaskaran/democicd"
